...
class TheLongestDescendingSequence {
	BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
	int n, max;
	int[] a, memo;
	
	TheLongestDescendingSequence(){
		try {
...
class Main{
	public static void main(String[] args) {
	  new TheLongestDescendingSequence().play();
	}
}
...
/*
	...
	이것은 [ 가장 긴 증가하는부분수열 ]에서 a[i] > a[j]이렇게 되어있는 기호만 바꾸면 된다. 
	...
	그리고 이중 for문을 돌리는데, 밑에 처럼 돌린다. 밑에 처럼 돌리면 어떤식으로 검사를 할 수 있는지 밑을 확인하자. 
		for i=1 < n
			for j=0 < i
				if a[i] < a[j] && memo[i] < memo[j] + 1
					memo[i] = memo[j] + 1
	10 30 10 20 20 10을 입력받고 위의 코드를 돌리면 
	
	      idx     |  1  |  2  |  3  |  4  |  5  |  6  |
	      a[i]    |  10 |  30 |  10 |  20 |  20 |  10 | 
	  초기 memo[i] |   1 |   1 |   1 |   1 |   1 |   1 |
	  최종 memo[i] |   1 |   1 |   2 |   2 |   2 |   2 |
	                                                3    
	위와 같이 값이 최종 memo[i]에 저장된다. 
	현재까지 구한 수가 이전에 구했던 값보다 작다면 memo[i]를 갱신하면서 큰 수를 찾는다. 
*/