...
/*
	...
	문제: 소설가인 김대전은 소설을 여러 장(chapter)으로 나누어 쓰는데, 각 장은 각각 다른 파일에 저장하곤 한다.
		소설의 모든 장을 쓰고 나서는 각 장이 쓰여진 파일을 합쳐서 최종적으로,
	 	소설의 완성본이 들어있는 한 개의 파일을 만든다. 이 과정에서 두 개의 파일을 합쳐서 하나의 임시파일을 만들고,
		...
		먼저 C2와 C3를 합쳐서 임시파일 X1을 만든다. 이 때 비용 60이 필요하다.
...
class MergeFiles {
	...
	void input() {
		try {
			...
			for(int i=1; strToken.hasMoreTokens()==true; i++) {
				int numb = Integer.parseInt(strToken.nextToken());
				sum[i] = sum[i-1]+numb;
			}
		}catch(IOException ie) {}
	}
...
/*
		한 줄 정리	
	소설의 장마다 따로 나눠놓았기 떄문에, 인접한 숫자만 합쳐야한다.
	이 부분에 대한 개념은 구간에 대한 개념으로 나누어 봐야 한다.
	1~n까지의 구간이 있다면, "1~n까지의 구간의 합을 최소로 만드는 것"은 임의의 j에 대하여 "1~j, j+1~n까지의 구간을 합쳐서" 그 합을 최소로 만든는 것과 같다.
*/
